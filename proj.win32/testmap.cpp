    ...
    int       nCmdShow)
{
    ...
    return TestmapPlayer::createAndRun();
}

int TestmapPlayer::createAndRun(void)
{
    TestmapPlayer *host = TestmapPlayer::sharedInstance();
    int ret = host->run();
    TestmapPlayer::purgeSharedInstance();
    return ret;
}

TestmapPlayer *TestmapPlayer::s_sharedInstance = NULL;

TestmapPlayer *TestmapPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new TestmapPlayer();
    }
    return s_sharedInstance;
}

void TestmapPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
TestmapPlayer::TestmapPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int TestmapPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "testmap-player");
...
void TestmapPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void TestmapPlayer::createViewMenu(void)
{
...
void TestmapPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void TestmapPlayer::updateWindowTitle(void)
{
}

void TestmapPlayer::relaunch(void)
{
...
void TestmapPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void TestmapPlayer::onFileOpenProject(void)
{
...
void TestmapPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"TestmapPlayer");
...
void TestmapPlayer::onFileProjectConfig(void)
{
...
void TestmapPlayer::onFileRelaunch(void)
{
    relaunch();
}

void TestmapPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void TestmapPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void TestmapPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void TestmapPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void TestmapPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT TestmapPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    TestmapPlayer *host = TestmapPlayer::sharedInstance();
...
INT_PTR CALLBACK TestmapPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
